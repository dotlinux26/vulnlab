...
const Subscription = () => {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      alert("Hệ thống thanh toán đang bảo trì!");
      setIsBuying(false);
    }
  };
      ...
      <main className="pt-32 pb-12 px-4 container mx-auto max-w-4xl">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-black uppercase tracking-tighter mb-4 text-transparent bg-clip-text bg-gradient-to-r from-primary to-emerald-500">
            Nâng cấp quyền truy cập Lab
          </h1>
          <p className="text-muted-foreground font-mono italic">Mở khóa chứng nhận nghề nghiệp với gói Voucher Pro.</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 items-start">
          ...
          <div className="bg-card border border-border p-8 rounded-3xl shadow-xl">
            <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
              <ShieldCheck className="text-primary" /> Quyền lợi gói Voucher Pro 
            </h2>
            <ul className="space-y-4">
              {[
                "2x Voucher",
                "Truy cập 100+ Lab Pentest thực tế",
                "Tải tài liệu Template báo cáo chuẩn nghiệp vụ",
                "Hỗ trợ Support 24/7 từ Admin",
                "Chứng chỉ kiểm tra năng lực nghề nghiệp (PDF)"
              ].map((item, i) => (
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-primary/5 border-2 border-primary rounded-3xl p-8 text-center relative overflow-hidden">
              ...
              <h3 className="text-lg font-bold text-muted-foreground uppercase mb-2">Voucher Pro 2X</h3>
            ...
            {isBuying && !voucherData && (
              <div className="bg-yellow-500/10 border border-yellow-500/30 p-4 rounded-2xl text-center animate-pulse">
                <p className="text-xs text-yellow-600 font-bold flex items-center justify-center gap-2">
                  <ExternalLink size={14} /> Hệ thống đang chờ tiền về...
                </p>
                <p className="text-[10px] text-muted-foreground mt-1">Sau khi chuyển khoản, đừng đóng trình duyệt nhé sếp!</p>
              </div>
            )}
          </div>
        </div>
