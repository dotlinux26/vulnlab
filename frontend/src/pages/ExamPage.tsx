...
const handleSubmitExam = async (
  ...
  if (!autoSubmit) {
    const confirmed = window.confirm(
      "Hành động này sẽ gửi bài cho Admin chấm. Chắc chắn nộp?"
    );
            ...
            return (
              <div key={exam.id} className={`bg-card p-6 rounded-2xl border transition-all shadow-sm hover:shadow-md ${isUnlocked ? 'border-primary/50' : 'border-border'}`}>
                <div className="flex justify-between items-start mb-4">
                  ...
                  {status === 'taking' && <span className="text-destructive flex items-center gap-1 font-bold text-sm"><Timer size={16} className="animate-pulse"/> ĐANG THI...</span>}
                  {status === 'pending' && <span className="text-yellow-500 flex items-center gap-1 font-bold text-sm"><Clock size={16}/> CHỜ CHẤM</span>}
                ...
                <div className="mt-auto pt-4 border-t border-border">
                  <button onClick={() => navigate(`/exams/${exam.id}`)} className="w-full bg-secondary hover:bg-secondary/80 text-white font-bold py-3 rounded-xl flex justify-center items-center gap-2 transition-colors border border-border">
                    XEM CHI TIẾT BÀI THI <ArrowRight size={18} />
                  </button>
                </div>
              </div>
            );
